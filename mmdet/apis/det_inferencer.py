...
class DetInferencer(BaseInferencer):
    ...
    def __init__(self,
                 model: Optional[Union[ModelType, str]] = None, # 配置文件的路径或 metafile 中定义的模型名称
                 weights: Optional[str] = None, # 模型权重文件的路径
                 device: Optional[str] = None, # 推理使用的设备
                 scope: Optional[str] = 'mmdet',  # 模型的”域名“
                 palette: str = 'none', #用于可视化的配色
                 show_progress: bool = True) -> None: #控制是否在推理过程中显示进度条。
    ...
    def __call__(
            self,
            inputs: InputsType, # 它可以是一个图片/文件夹的路径，一个 numpy 数组，或者是一个包含图片路径或 numpy 数组的列表/元组
            batch_size: int = 1, # 推理的批大小
            return_vis: bool = False, # 是否返回可视化结果
            show: bool = False, # 是否在弹出窗口中显示可视化结果
            wait_time: int = 0, # 弹窗展示可视化结果的时间间隔
            no_save_vis: bool = False, # 是否将可视化结果保存到 out_dir
            ...
            out_dir: str = '', # 结果的输出目录
            # by open image task
            texts: Optional[Union[str, list]] = None, # 文本提示词
            # by open panoptic task
            stuff_texts: Optional[Union[str, list]] = None, # 物体文本提示词
            # by GLIP
